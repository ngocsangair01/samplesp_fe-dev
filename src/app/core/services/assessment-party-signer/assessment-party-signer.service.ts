...
export class AssessmentPartySignerService extends BasicService {
  constructor(public httpClient: HttpClient, public helperService: HelperService) {
    super('assessment', 'assessmentPartySigner', httpClient, helperService );
  }

  public getInfo(assessmentPartySignerId: number): Observable<any> {
    const url = `${this.serviceUrl}/get-info/${assessmentPartySignerId}`;
    return this.getRequest(url);
  }

  public downloadTemplateImport(assessmentPartySignerId: number): Observable<any> {
    const url = `${this.serviceUrl}/download-template-import`;
    return this.getRequest(url, {params: {assessmentPartySignerId: assessmentPartySignerId}, responseType: 'blob'});
  }
  ...
  public getBaseInforDetail(assessmentPartySignerId: number): Observable<any> {
    const url = `${this.serviceUrl}/get-base-infor-detail/${assessmentPartySignerId}`;
    return this.getRequest(url);
  }

  public getSignFileInformation(assessmentPartySignerId: number): Observable<any>{
    const url = `${this.serviceUrl}/get-sign-file-information/${assessmentPartySignerId}`;
    return this.getRequest(url);
  }
