...
@Component({
  selector: 'vicinity-position-plan-search-clone',
  templateUrl: './vicinity-position-plan-search.component.html'
})
export class VicinityPositionPlanSearchCloneComponent extends BaseComponent implements OnInit {
  ...
  public prepareSaveOrUpdate(item?: any) {
    if (item && item.vicinityPositionPlanId > 0) {
      this.vicinityPositionPlanService.findOne(item.vicinityPositionPlanId)
        .subscribe(res => {
          if (res.data != null) {
            this.router.navigate(['/employee/vicinity-position-plan-clone/edit/', item.vicinityPositionPlanId]);
          }
        })
    }
    else {
      this.router.navigate(['/employee/vicinity-position-plan-clone/add']);
    }
  }

  processDelete(vicinityPositionPlanId) {
    if (vicinityPositionPlanId && vicinityPositionPlanId > 0) {
      this.app.confirmDelete(null, () => {// on accepted
        this.vicinityPositionPlanService.deleteById(vicinityPositionPlanId)
          .subscribe(res => {
  ...
  prepareImport() {
    this.router.navigate(['/employee/vicinity-position-plan-clone/import']);
  }
  //dunglv upgarde
  public getDetaiLById(item?: any) {
    if (item && item.vicinityPositionPlanId > 0) {
      this.vicinityPositionPlanService.getDetaiLById(item.vicinityPositionPlanId)
        .subscribe(res => {
          if (res.data != null) {
            this.router.navigate(['/employee/vicinity-position-plan-clone/detail/', item.vicinityPositionPlanId]);
          }
        })
    }
  }
