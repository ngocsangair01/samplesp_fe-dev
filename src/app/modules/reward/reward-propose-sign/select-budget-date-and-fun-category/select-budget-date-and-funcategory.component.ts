...
@Component({
  ...
  templateUrl: './select-budget-date-and-fun-category.component.html',
  styleUrls: ['./select-budget-date-and-fun-category.component.css']
})
export class SelectBudgetDateAndFundCategoryComponent extends BaseComponent implements OnInit {
  formSearch: FormGroup;
  lstReason: any;
  selectedRows: [];
  formConfig = {
    budgetDate: [null, [ValidationService.required]],
    funCategory: [null, [ValidationService.required]],
    autoPayOrder: [false]
  };
  ...
  public setFormValue(propertyConfigs: any, data?: any) {
    ...
    if(data.formSearch.rewardType === 1){
      this.lstfunCategory = APP_CONSTANTS.REWARD_FUN_CATEGORY_LIST_OTHER;
      this.formSearch.controls['funCategory'].setValue('F72')
    }else{
      this.lstfunCategory = APP_CONSTANTS.REWARD_FUN_CATEGORY_LIST;
      if(!this.formSearch.controls['funCategory'].value){
        this.formSearch.controls['funCategory'].setValue('F03')
      }
    }
  }
  ...
  processImport() {
    ...
    if (!saveData.funCategory) {
      this.app.errorMessage("rewardPropose.FunCategory");
      return;
    }


    this.rewardProposeSignService.processTransferPayment(rewardForm)
      .subscribe(res => {
        if (this.rewardProposeSignService.requestIsSuccess(res)) {
          this.activeModal.close();
          // this.router.navigate(['/reward/reward-propose-sign/decided-sign-outside/view-decision/', this.rewardProposeSignId]);
