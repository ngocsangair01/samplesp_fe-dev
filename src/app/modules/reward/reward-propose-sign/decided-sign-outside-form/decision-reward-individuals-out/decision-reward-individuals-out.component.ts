...
@Component({
  selector: 'decision-reward-individuals-out',
  templateUrl: './decision-reward-individuals-out.component.html',
  styleUrls: ['./decision-reward-individuals-out.component.css']
})
export class DecisionRewardIndividualsOutComponent extends BaseComponent implements OnInit {
  ...
  @Input() rewardType;
  @Input() importStaffOutsangnn: Subject<any> = new Subject<any>();
  importStaffOutsangnnSubscription: Subscription;
  ...
  ngOnDestroy() {
    ...
    this.importStaffOutsangnnSubscription.unsubscribe();
  }
  ngOnInit() {
    ...
    this.importStaffOutsangnnSubscription = this.importStaffOutsangnn.subscribe(async res => {
      await this.makeMapRewardTitleIdList();
    ...
    for (const key in dataList) {
      ...
      if (element.rewardProposeDetailType == null) {
        group.patchValue({
          ...
          bank: element.bank,
          objectNameGuest: element.lstRewardProposeSignObjectForm[0].objectNameGuest,
          accountNumber: element.accountNumber,
          nationId: element.lstRewardProposeSignObjectForm[0].nationId,
          residencyStatus: element.lstRewardProposeSignObjectForm[0].residencyStatus,
          personalIdIssuedPlace: element.lstRewardProposeSignObjectForm[0].personalIdIssuedPlace,
          addressOrPhone: element.lstRewardProposeSignObjectForm[0].addressOrPhone,
          personalIdIssuedDate: element.lstRewardProposeSignObjectForm[0].personalIdIssuedDate,
          personalIdNumber: element.lstRewardProposeSignObjectForm[0].personalIdNumber,
          rewardCategory: element.rewardCategory.toString(),
          rewardTitleName: element.rewardTitleName,
          nationName: this.nationList!=null && this.nationList.find(e => e.nationId == element.nationId) ? this.nationList.find(e => e.nationId == element.nationId).name : "",
          taxId: element.lstRewardProposeSignObjectForm[0].taxId,
