...
@Component({
  selector: 'email-sms-history-bk-form',
  templateUrl: './email-sms-history-bk-form.component.html',
})

export class EmailSmsHistoryBkFormComponent extends BaseComponent implements OnInit {
  formSave: any;
  ...
  ngOnInit() {
    ...
    if (subPaths.length > 2) {
      this.isView = subPaths[2] === 'email-sms-history-bk-view';
      this.isEdit = subPaths[2] === 'email-sms-history-bk-edit';
      this.isAdd = subPaths[2] === 'email-sms-history-bk-add';
    }
  ...
  public goBack() {
    this.router.navigate(['/settings/email-sms-history-bk']);
  }
