...
@Component({
  selector: 'party-member-form',
  templateUrl: './party-member-form.component.html',
  styleUrls: ['./party-member-form.component.css']
})
export class PartyMemberFormComponent extends BaseComponent implements OnInit {
  ...
  public goView(employeeId: any) {
    this.router.navigate([`/party-organization/party-member/curriculum-vitae/${employeeId}/view`]);
  }
  ...
  public processSaveOrUpdate() {
    // this.formPartyMember.updateValueAndValidity();
    ...
    this.app.confirmMessage(null, () => { // on accepted
      ...
      this.partyMemebersService.saveOrUpdate(formSave)
        .subscribe(res => {
          if (this.partyMemebersService.requestIsSuccess(res) && res.data && res.data.employeeId) {
            this.goView(res.data.employeeId);
          }
        });
    }, () => {
      // on rejected
