...
@Component({
  selector: 'party-member-increase',
  templateUrl: './party-member-increase.component.html',
  styleUrls: ['../../party-home.component.css']
})
export class PartyMemberIncreaseComponent extends BaseComponent implements OnInit {
   ...
   */
  public getPartyIncreaseMember(partyOrganizationId?: number ){
    this.partyOrganizationId = partyOrganizationId? partyOrganizationId: 0;
    const type = this.monthType.value;
    this.partyHomeService.getPartyIncreaseMember({monthType: type, partyOrganizationId: this.partyOrganizationId}).subscribe(res => {
      this.resultList = res;
