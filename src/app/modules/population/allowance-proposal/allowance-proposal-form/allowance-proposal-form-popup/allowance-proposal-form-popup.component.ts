...
@Component({
  selector: 'allowance-proposal-form-popup',
  templateUrl: './allowance-proposal-form-popup.component.html',
  styleUrls: ['./allowance-proposal-form-popup.component.css']
})
export class AllowanceProposalFormPopupComponent extends BaseComponent implements OnInit{
    formSave: FormGroup;
    ...
    callDataForTable(){
        if(this.f['allowanceProposalBOList'].value.length > 0 && this.f['allowanceProposalBOList'].value[0].allowanceProposalId){
            ...
            this.allowanceProposalService.getAllowanceProposalTableBean(
                {proposeOrgId: this.f['proposeOrgId'].value, list: listLong,
