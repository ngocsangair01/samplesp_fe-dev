...
} from "@app/modules/competition-program/competition-program-form-send/competition-program-form-send.component";
...
@Component({
    selector: 'competition-program-form',
    templateUrl: './competition-program-form.component.html',
    styleUrls: ['./competition-program-form.component.css']
})
export class CompetitionProgramFormComponent extends BaseComponent implements OnInit {
    formSave: FormGroup;
    view: boolean;
    update: boolean;
    firstTitle: any;
    ...
    public goBack() {
        if (this.view) {
            if (this.type === 'competition-registration') {
                this.router.navigate(['/competition-unit-registration']);
            } else if (this.type === 'competition-program') {
                this.router.navigate(['/competition-program']);
            } else {
    ...
    public goView(competitionId: any) {
        this.router.navigate([`/competition-program/view/competition-program/${competitionId}`]);
    }
    ...
    handleBr5(): boolean {
        ...
        if (this.applicableTypeAnnual && CommonUtils.isNullOrEmpty(this.formSave.get('competitionTitleCode').value)) {
            this.formSave.controls['competitionTitleCode'].setErrors({required: true});
            isInvalid = true;
        } else if (!this.applicableTypeAnnual && CommonUtils.isNullOrEmpty(this.formSave.get('competitionMethodCode').value)) {
    ...
    public processSaveOrUpdate() {
        ...
        this.app.confirmMessage(null, () => { // on accepted
            let data = {
                ...
                programType: 1, // loại chương trình thi đua
            }
    ...
    sendMessage() {
        if (new Date().getTime() < this.formSave.get('startTime').value || new Date().getTime() > new Date(new Date(this.formSave.get('endTime').value).getTime() + (1000 * 60 * 60 * 24)).getTime()) {
            ...
            const ref = this.dialogService.open(CompetitionProgramFormSendComponent, {
