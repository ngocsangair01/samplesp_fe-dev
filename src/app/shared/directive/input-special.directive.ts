...
@Directive({
  selector: 'input[type="text"],input[type="email"],input[type="tel"],input:not([othertype="url"]),input:not([othertype="textWithSpec"])',
  providers: [
    {
...
export class InputSpecialDirective implements Validator {
  ...
  validateSpecialCharecter() {
    return (c: FormControl) => {
      const text = c.value;
      if (text && !Array.isArray(text) && (this._type === 'text' || this._type === 'email' || this._type === 'tel') && !this._othertype) {
        const iChars = '!#$^*[]\\{}\"?<>\'';
