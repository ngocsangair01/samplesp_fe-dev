...
@Component({
  selector: 'select-icon-filter',
  templateUrl: './select-icon-filter.component.html',
})
export class SelectIconFilterComponent implements OnChanges, OnInit, AfterViewInit {
  ...
  public filter = true;
  @Input()
  public isDisplayClear = true;
  @Input()
  public rewardGroupType: any;
  ...
  public setRewardGroupType(item: any) {
    if(this.rewardGroupType) {
      const currentItem = this.rewardGroupType.find(x => x.rewardCategoryId == item.value);
      return currentItem.rewardCategory;
    }
    return 0;   
  }
}
