...
@Component({
  selector: 'multi-org-selector-modal',
  templateUrl: './multi-org-selector-modal.component.html',
})
export class MultiOrgSelectorModalComponent implements OnInit {
  
  nodes: TreeNode[];
  selectedNode: TreeNode;
  showOrgExpried = false;
  fillterShowOrgExpried = ' CURDATE() BETWEEN o.effective_start_date and IFNULL(o.effective_end_date, CURDATE())';
  ...
  processSearch(event) {
    if (CommonUtils.isValidForm(this.form)) {
      ...
      if (this.showOrgExpried) {
        params.filterCondition = this.defaultFillter;
      } else {
        if (this.defaultFillter && this.fillterShowOrgExpried) {
          params.filterCondition = this.defaultFillter + ' AND ' + this.fillterShowOrgExpried;
        } else {
          params.filterCondition = this.fillterShowOrgExpried;
        }
      }
      ...
      this.service.search(params, event).subscribe(res => {
        ...
        for (let data of this.resultList.data) {
          for (let objSelect of this.list) {
            if (data.organizationId === objSelect.organizationId) {
              data.check = true;
            }
          }
        }
      ...
      this.orgService.findOne(event.node.nodeId)
        .subscribe(res => {
          if (this.orgService.requestIsSuccess(res)) {
            item = { organizationId: res.data.organizationId, code: res.data.code, name: res.data.name, check: true };
            let index = this.list.findIndex(x => x.organizationId === item.organizationId);
            if (index < 0) {
              this.list.push(item);
            }
            this.chose();
  ...
  chooseAll(event){
    if(this.resultList.data.length > 0){
      for(const item in this.resultList.data){
        // xác định vị trí xem có tồn tại chưa
        let index = this.list.findIndex(x => x.organizationId === this.resultList.data[item].organizationId);
        // chưa tồn tại và check all
        if (index < 0 && event.currentTarget.checked) {
          this.list.push(this.resultList.data[item])
        }
        // đã tồn tại và bỏ check all
        if (index >= 0 && !event.currentTarget.checked){
          this.list.splice(index, 1);
        }
      }
