...
import { ReportMultiPartyOrgSelectorModalComponent } from './report-multi-party-org-selector-modal/report-multi-party-org-selector-modal.component';

@Component({
  selector: 'report-multi-party-org-selector',
  templateUrl: './report-multi-party-org-selector.component.html',
  styleUrls: ['./report-multi-party-org-selector.component.scss'],
})
export class ReportMultiPartyOrgSelectorComponent implements OnChanges {
  @Input()
  ...
  public onChangeOrgId() {
    ...
    for (const iterator of this.property.value) {
      this.service.findOne(iterator)
        .subscribe((res) => {
          const data = res.data;
          list.push({ partyOrganizationId: data.partyOrganizationId, code: data.code, name: data.name, check: true, nodeId: data.partyOrganizationId });
        });
    }
    this.list = list;
  }
   ...
   */
  public onChose() {
    const modalRef = this.modalService.open(ReportMultiPartyOrgSelectorModalComponent, DEFAULT_MODAL_OPTIONS);
    ...
    modalRef.result.then((item) => {
      ...
      item.map(x => x.partyOrganizationId).forEach(element => {
  ...
  removeTag(id) {
    this.list = this.list.filter(item => (item.partyOrganizationId !== id))
    this.property = CommonUtils.clearFormArray(this.property);
    this.list.forEach(element => {
      this.property.push(this.fb.control(element.partyOrganizationId));
    });
