...
@Component({
  selector: 'report-multi-org-selector-modal',
  templateUrl: './report-multi-org-selector-modal.component.html',
})
export class ReportMultiOrgSelectorModalComponent implements OnInit {
  nodes: TreeNode[];
      ...
      this.service.search(params, event).subscribe(res => {
        ...
        if (this.listSelectedNode && this.listSelectedNode instanceof Array) {
          ...
          this.list.forEach(ele => {
            mapOrgId[ele.organizationId] = ele;
          });
          ...
          if (this.resultList && this.resultList.data instanceof Array) {
            this.resultList.data.forEach((item: any) => {
              ...
              const mapItem = mapOrgId[organizationId];
              if (mapItem) {
                item.check = true;
              }
            });
  ...
  nodeSelect(event) {
    ...
    if (lstNodes.length) {
      lstNodes.forEach(item => {
        this.listSelectedNode.push(item);
        const newItem = { organizationId: item.nodeId, nodeId: item.nodeId, name: item.label, check: true };
  ...
  nodeUnSelect(event) {
    const indexOf = this.list.findIndex(ele => ele.organizationId === event.node.nodeId);
    if (indexOf !== -1) {
  ...
  tickOrOffSearchList() {
    ...
    if (listChange && listChange.data instanceof Array) {
      const selectMap = {};
      this.list.forEach(ele => {
        selectMap[ele.organizationId] = ele;
      });
      listChange.data.forEach(ele => {
        const check = selectMap[ele.organizationId];
        ele.check = check;
      });
  ...
  findDataNode(data, parent) {
    if (parent && parent instanceof Array && parent.length) {
      for (const key in parent) {
        if (parent[key].nodeId === data.organizationId) {
          return parent[key];
  ...
  change(event, item) {
    const indexNodeSelect = this.listSelectedNode.findIndex((ele: any) => ele.organizationId === item.organizationId);
