...
import {DataPickerService, PartyOrganizationService} from '@app/core';
import { CommonUtils, CryptoService } from '@app/shared/services';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';

@Component({
  selector: 'parent-training-topic-picker-modal',
  templateUrl: './parent-training-topic-picker-modal.component.html',
})
export class ParentTrainingTopicPickerModalComponent {
  @Input()
  params: any;
  form: FormGroup;
  resultList: any = {};
  ...
  setInitValue(params: {
    ...
    this.params.orderField = CryptoService.encrAesEcb(this.params.orderField);
  ...
  public processSearch(event) {
    if (CommonUtils.isValidForm(this.form)) {
      ...
      this.service.searchThoroughContent(paramsSearch, event).subscribe(res => {
        this.resultList = res;
      });
    }
  }
  public processSearchTimeout() {
    ...
    this.fnSearch = setTimeout(() => {
      ...
      this.service.searchThoroughContent(paramsSearch).subscribe(res => {
        this.resultList = res;
      });
    }, 1000);
  }
